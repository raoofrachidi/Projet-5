"""Class product of table Product
"""
...
class Product:
    ...
    def insert_data(self, name, url, grade, id_category, store):
        """
        Method that inserts data from API to Product table
        """
    ...
    def get_product(self, index):
        ...
        return list of all products from index category
        """
        query = (
    ...
    def search_product(self, grade, index_category):
        """ Method that displays substitute products
        return list of products with a better nutritional quality
        """
        query = (
            "SELECT `*` FROM `Product`"
            "INNER JOIN `Category`"
